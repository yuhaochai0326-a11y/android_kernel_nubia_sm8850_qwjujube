		...
		off_t sector_size)
{
	...
	int i;
	exfat_error("verify_vbr_checksum enter!");
	...
	exfat_error("verify_vbr_checksum exit!");
	return true;
}
...
int exfat_mount(struct exfat* ef, const char* spec, const char* options)
{
	...
	exfat_error("exfat_error:exfat_mount enter...");
	exfat_tzset();
	...
	exfat_error("call exfat_open...");
	ef->dev = exfat_open(spec, mode);
	if (ef->dev == NULL) {
		exfat_error("ef->dev is NULL");
		return -EIO;
    }

	exfat_error("call exfat_get_mode...");
	...
	exfat_error("call exfat_pread for super block...");
	...
	exfat_error("call verify_vbr_checksum...");
	...
	exfat_error("call exfat_get_node ...");
	exfat_get_node(ef->root);
	exfat_error("call exfat_cache_directory ...");
	...
	exfat_error("call prepare_super_block ...");
	if (prepare_super_block(ef) != 0)
		goto error;
  
	exfat_error("exfat_error:exfat_mount exit...");
	return 0;

error:
	exfat_error("deal with error info ...");
