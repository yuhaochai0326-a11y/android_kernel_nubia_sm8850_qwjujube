				...
				enum dsi_cmd_set_type type, bool do_peripheral_flush)
{
	...
	if (mode->priv_info->cmd_sets[type].logable)
		pr_info("MSM_LCD %s panel cmds [id, type, count] = [%d, %s, %d]\n", panel->name, type, mode->priv_info->cmd_sets[type].name, count); /*add by zte for log print*/
	#endif
...
extern int zte_dsi_panel_update_backlight(struct dsi_panel *panel, u32 bl_lvl);
#endif
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"zte,mdss-dsi-dim-off-commands-state",
