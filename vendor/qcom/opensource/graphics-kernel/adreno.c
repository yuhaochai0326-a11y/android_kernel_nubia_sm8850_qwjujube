...
extern int __init qcom_iommu_util_init(void);
extern void qcom_iommu_util_exit(void);
extern int __init mem_buf_init(void);
extern void mem_buf_exit(void);

static int __init kgsl_3d_init(void)
{
	int ret;

	ret = qcom_iommu_util_init();
	if (ret)
		return ret;

	ret = mem_buf_init();
	if (ret) {
		qcom_iommu_util_exit();
		return ret;
	}
...
MODULE_DESCRIPTION("RedMagic 11 Pro Stabilized GPU Driver (Adreno 840)");
