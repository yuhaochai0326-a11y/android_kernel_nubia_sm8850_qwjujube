...
int __init qcom_iommu_util_init(void)
{
	initcall_t *init_fn;
	exitcall_t *exit_fn;
	int ret;
...
void qcom_iommu_util_exit(void)
{
	exitcall_t *exit_fn;

	for (exit_fn = exit_table; *exit_fn; exit_fn++) {
		if (*exit_fn)
			(**exit_fn)();
	}
}
// Removed initcalls to allow manual initialization from KGSL main driver
