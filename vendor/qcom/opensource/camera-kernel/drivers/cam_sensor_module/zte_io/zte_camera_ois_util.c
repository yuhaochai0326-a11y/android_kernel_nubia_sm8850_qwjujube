...
static int ois_debugfs_cal_g(void *data, u64 *val)
{
	msm_ois_debug_info_t *ptr = (msm_ois_debug_info_t *) data;
  	...
  	if(strncmp(ptr->s_ctrl->ois_name, "ois_ov64b40_qwbarley", strlen("ois_ov64b40_qwbarley")) == 0) {
          CDBG("%s:%d: %s start cal E", __func__, __LINE__, ptr->s_ctrl->ois_name);
  		ois_dw9784_debugfs_cal_g(data, val);
  	} else if (strncmp(ptr->s_ctrl->ois_name, "ois_dw9785_qwjujube", strlen("ois_dw9785_qwjujube")) == 0) {
          CDBG("%s:%d: %s start cal E", __func__, __LINE__, ptr->s_ctrl->ois_name);
  		ois_dw9785_debugfs_cal_g(data, val);
  	} else {
