...
static int32_t cam_sensor_trigger_temperature_update(
	struct camera_io_master *io_master_info)
{
	int32_t ret = 0;

	struct cam_sensor_i2c_reg_array temperature_setting = {
		.reg_addr = 0x4D12,
		.reg_data = 0x01,
		.delay = 0x00U,
		.data_mask = 0x00U,
	...
	struct cam_sensor_i2c_reg_setting temperature_apply_setting = {
		.size = 1,
		.reg_setting = &temperature_setting,
		.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD,
		.data_type = CAMERA_SENSOR_I2C_TYPE_BYTE,
		.delay = 0,
	};

	ret = camera_io_dev_write_continuous(io_master_info, &temperature_apply_setting, CAM_SENSOR_I2C_WRITE_SEQ);
	...
	struct camera_io_master *io_master_info)
{
	...
	list_for_each_entry(i2c_list,
		...
		if (i2c_list->op_code == CAM_SENSOR_I2C_READ_RANDOM) {
			...
			if ((strncmp(io_master_info->sensor_name, "ov64b40", strlen("ov64b40")) == 0)
				&& i2c_list->i2c_settings.reg_setting[0].reg_addr == 0x4D13) {
				rc = cam_sensor_trigger_temperature_update(io_master_info);
				if (rc < 0) {
					CAM_ERR(CAM_SENSOR_UTIL,
					"Failed to write sensor temperature I2C settings: %d",
					rc);
					return rc;
				}
			}
