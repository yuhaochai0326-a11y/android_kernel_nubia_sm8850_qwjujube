...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_OIS_PACKET_OPCODE_WRITE_TIME: {
		...
		if (!o_ctrl->check_gyro_raw_valid_done &&
			!strncmp(o_ctrl->ois_name, "ois_ov64b40_qwbarley", sizeof("ois_ov64b40_qwbarley") - 1)) {
			uint32_t  x_gyro_raw = 0;
			...
			if (rc < 0) {
				CAM_DBG(CAM_OIS, "%s read x gyro raw failed", o_ctrl->ois_name);
			} else {
				CAM_DBG(CAM_OIS, "%s read x gyro raw 0x%x", o_ctrl->ois_name, x_gyro_raw);
				if (x_gyro_raw == 0) {
					CAM_ERR(CAM_OIS, "%s read x gyro raw is 0, config again", o_ctrl->ois_name);
					...
					if (rc < 0) {
						CAM_ERR(CAM_OIS, "dw9784 set 0x7020 failed");
					} else {
						o_ctrl->check_gyro_raw_valid_done = true;
						CAM_DBG(CAM_OIS, "dw9784 gyro raw config done!");
					}
				} else {
					o_ctrl->check_gyro_raw_valid_done = true;
					CAM_DBG(CAM_OIS, "dw9784 gyro raw is valid!");
				}
			}
		}
