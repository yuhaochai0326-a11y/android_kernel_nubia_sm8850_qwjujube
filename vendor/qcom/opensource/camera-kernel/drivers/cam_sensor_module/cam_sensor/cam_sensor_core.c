...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	for (uint8_t i = 0; i < g_CameraOisParamsCount; i++)
	{
		if (!s_ctrl->ois_pdev || strncmp(s_ctrl->sensor_name, g_CameraOisParams[i].SensorName, strlen(s_ctrl->sensor_name)) != 0)
		{
			continue;
		}

		struct cam_ois_ctrl_t *o_ctrl = platform_get_drvdata(s_ctrl->ois_pdev);
		if (!o_ctrl || !o_ctrl->read_ver) {
			CAM_ERR(CAM_SENSOR, "OIS device not found or unsupported");
			continue;
		}

		snprintf(o_ctrl->ois_name, OIS_NAME_LEN, "%s", g_CameraOisParams[i].OISName);
		if (cam_ois_dw978x_need_update_fw(o_ctrl)) {
			memcpy(o_ctrl->io_master_info.cci_client, s_ctrl->io_master_info.cci_client, sizeof(struct cam_sensor_cci_client));
			uint32_t current_fw_ver = o_ctrl->read_ver(o_ctrl,  g_CameraOisParams[i].config);
			uint32_t target_fw_ver = g_CameraOisParams[i].FWVersion << 16 | g_CameraOisParams[i].FWDate;

			if (current_fw_ver != 0 && current_fw_ver != INVAILD_FW_VER &&
				(current_fw_ver & 0xFFFF0000) < (target_fw_ver & 0xFFFF0000)) {
				// update ois firmware
				CAM_ERR(CAM_SENSOR, "%s: Updating OIS firmware from 0x%x to 0x%x",
					o_ctrl->ois_name, current_fw_ver, target_fw_ver);

				current_fw_ver = o_ctrl->dw978x_firmware_download(o_ctrl, g_CameraOisParams[i].config);
				if (current_fw_ver == target_fw_ver) {
					CAM_ERR(CAM_SENSOR, "%s: Firmware update successful (current 0x%x vs target 0x%x)",
							o_ctrl->ois_name, current_fw_ver, target_fw_ver);
				} else {
					CAM_ERR(CAM_SENSOR, "%s: Firmware update failed (current 0x%x vs target 0x%x)",
							o_ctrl->ois_name, current_fw_ver, target_fw_ver);
				}
			} else {
				CAM_ERR(CAM_SENSOR, "%s: no need update ois firmware (version 0x%x)",
						o_ctrl->ois_name, current_fw_ver);
			}
		} else {
			CAM_ERR(CAM_SENSOR, "%s: OIS firmware disable update", o_ctrl->ois_name);
		}
	}
#endif
