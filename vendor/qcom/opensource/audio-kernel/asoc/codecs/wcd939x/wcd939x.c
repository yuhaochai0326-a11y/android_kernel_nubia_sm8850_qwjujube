...
static int bmic_sw_control = 0; /* ZTE_chenjun */
...
/* ZTE_chenjun: Start */
static int bmic_sw_control_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: bmic_sw_control = %d\n",
			 __func__, bmic_sw_control);

	ucontrol->value.enumerated.item[0] = bmic_sw_control;

	return 0;
}

static int bmic_sw_control_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
					snd_soc_kcontrol_component(kcontrol);
	...
	dev_info(component->dev, "%s: status = %d\n", __func__, status);

	if (pdata->bmic_sw_gpio < 0) {
		pr_err("%s: bmic_sw_gpio %d is invalid\n",
			__func__, pdata->bmic_sw_gpio);

		return -EINVAL;
	}

	 if (status) {
		ret = gpio_direction_output(pdata->bmic_sw_gpio, 1);
		if (ret < 0) {
			pr_err("%s(): bmic_sw_gpio direction failed %d\n",
				__func__, ret);
			return ret;
		}
	 } else {
		ret = gpio_direction_output(pdata->bmic_sw_gpio, 0);
		if (ret < 0) {
			pr_err("%s(): bmic_sw_gpio direction failed %d\n",
				__func__, ret);
			return ret;
...
/* ZTE_chenjun: Start */
	SOC_SINGLE_EXT("Back Mic Switch", SND_SOC_NOPM, 0, 1, 0,
			bmic_sw_control_get, bmic_sw_control_put),
/* ZTE_chenjun: End */
...
/* ZTE_chenjun, begin */
	pdata->bmic_sw_gpio = of_get_named_gpio(dev->of_node,
						"qcom,bmic-sw-gpio", 0);
	if (pdata->bmic_sw_gpio < 0) {
		dev_err(dev, "%s: %s property not found %d\n",
			__func__, "qcom,bmic-sw-gpio", pdata->bmic_sw_gpio);
	} else {
		ret = gpio_request(pdata->bmic_sw_gpio, "bmic_sw_gpio");
		if (ret) {
			dev_err(dev,
				"%s: bmic_sw_gpio request failed, ret:%d\n",
				__func__, ret);
		}
		gpio_direction_output(pdata->bmic_sw_gpio, 0); /* 0: top mic, 1: back mic */
		bmic_sw_control = 0;
	}
/* ZTE_chenjun, end */
